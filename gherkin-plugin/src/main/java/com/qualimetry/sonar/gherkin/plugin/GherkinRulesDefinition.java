 ...
 * <p>
 * Loads all 86 rule check classes from {@link CheckList}, sets their
...
public class GherkinRulesDefinition implements RulesDefinition {
    ...
    private static final Map<String, RuleType> RULE_TYPES = Map.ofEntries(
            ...
            Map.entry("no-empty-data-table-cells", RuleType.BUG)
    );

    /**
     * Severity per rule key (Sonar standard: BLOCKER, CRITICAL, MAJOR, MINOR, INFO).
     */
    private static final Map<String, String> SEVERITIES = Map.ofEntries(
            ...
            // Rules 84-86
            Map.entry("max-line-length", Severity.MINOR),
            Map.entry("no-empty-data-table-cells", Severity.MAJOR),
            Map.entry("examples-constant-column", Severity.MINOR)
    );
    ...
    private static final Map<String, String> DISPLAY_NAMES = Map.ofEntries(
            ...
            // Rules 84-86
            Map.entry("max-line-length", "Max Line Length"),
            Map.entry("no-empty-data-table-cells", "No Empty Data Table Cells"),
            Map.entry("examples-constant-column", "Examples Constant Column")
    );
    ...
    private static final Map<String, String> REMEDIATIONS = Map.ofEntries(
            ...
            // Rules 84-86
            Map.entry("max-line-length", "2min"),
            Map.entry("no-empty-data-table-cells", "5min"),
            Map.entry("examples-constant-column", "10min")
    );

    /**
     * Base tag applied to every rule (language identifier).
     * Only custom tag used; all others are SonarQube built-in tags.
     */
    private static final String[] BASE_TAGS = {"gherkin"};

    /**
     * Additional tags per rule. Only SonarQube built-in tags: convention, design, suspicious, bad-practice, pitfall, unused.
     * No security/CWE/OWASP: none of our rules cover credentials, HTTPS, permissions, etc.
     */
    private static final Map<String, String[]> RULE_TAGS = buildRuleTags();

    private static Map<String, String[]> buildRuleTags() {
        String[] convention = {"convention"};
        String[] design = {"design"};
        String[] conventionDesign = {"convention", "design"};
        String[] suspicious = {"suspicious"};
        String[] conventionPitfall = {"convention", "pitfall"};

        return Map.ofEntries(
                // Structure (1-10, 80, 82)
                Map.entry("feature-file-required", convention),
                Map.entry("feature-name-required", convention),
                Map.entry("feature-description-recommended", convention),
                Map.entry("scenario-required", convention),
                Map.entry("scenario-name-required", convention),
                Map.entry("step-required", convention),
                Map.entry("examples-minimum-rows", convention),
                Map.entry("examples-column-coverage", convention),
                Map.entry("scenario-count-limit", convention),
                Map.entry("step-count-limit", convention),
                Map.entry("feature-name-matches-filename", convention),
                Map.entry("no-empty-doc-strings", convention),
                // Design (11-21, 77)
                Map.entry("background-given-only", design),
                Map.entry("shared-given-to-background", design),
                Map.entry("step-order-given-when-then", design),
                Map.entry("single-when-per-scenario", design),
                Map.entry("when-then-required", design),
                Map.entry("unique-feature-name", design),
                Map.entry("unique-scenario-name", design),
                Map.entry("no-duplicate-steps", design),
                Map.entry("use-scenario-outline-for-examples", design),
                Map.entry("business-language-only", design),
                Map.entry("consistent-feature-language", design),
                Map.entry("no-commented-out-steps", design),
                // Style and Convention (22-33, 81)
                Map.entry("consistent-indentation", convention),
                Map.entry("no-tab-characters", convention),
                Map.entry("no-trailing-whitespace", convention),
                Map.entry("newline-at-end-of-file", convention),
                Map.entry("no-byte-order-mark", convention),
                Map.entry("consistent-line-endings", convention),
                Map.entry("file-name-convention", convention),
                Map.entry("comment-format", convention),
                Map.entry("prefer-and-but-keywords", convention),
                Map.entry("no-star-step-prefix", convention),
                Map.entry("examples-separator-line", convention),
                Map.entry("step-sentence-max-length", convention),
                Map.entry("no-multiple-empty-lines", convention),
                // Tag (34-38, 78)
                Map.entry("tag-name-pattern", convention),
                Map.entry("tag-permitted-values", convention),
                Map.entry("tag-placement", convention),
                Map.entry("no-redundant-tags", convention),
                Map.entry("no-examples-tags", convention),
                Map.entry("no-partially-commented-tag-lines", convention),
                // Variable and Data (39, 75-76)
                Map.entry("no-unused-variables", new String[]{"unused", "suspicious"}),
                Map.entry("no-empty-examples-cells", suspicious),
                Map.entry("unique-examples-headers", convention),
                // Step Pattern (40-43)
                Map.entry("given-step-pattern", convention),
                Map.entry("when-step-pattern", convention),
                Map.entry("then-step-pattern", convention),
                Map.entry("no-unknown-step-type", suspicious),
                // Comment and Marker (44-46, 79)
                Map.entry("todo-comment", convention),
                Map.entry("fixme-comment", convention),
                Map.entry("comment-pattern-match", convention),
                Map.entry("spelling-accuracy", convention),
                // Parser Error (48)
                Map.entry("parse-error", suspicious),
                // Rule Block Quality (49-52)
                Map.entry("rule-name-required", conventionDesign),
                Map.entry("rule-scenario-required", conventionDesign),
                Map.entry("unique-rule-name", conventionDesign),
                Map.entry("rule-description-recommended", conventionDesign),
                // Structural Integrity (53-56, 83)
                Map.entry("outline-placeholder-required", suspicious),
                Map.entry("scenario-outline-requires-examples", suspicious),
                Map.entry("background-needs-multiple-scenarios", design),
                Map.entry("blank-line-before-scenario", convention),
                Map.entry("background-step-count-limit", convention),
                // Rule-Scoped Best Practices (57-60)
                Map.entry("rule-scenario-count-limit", conventionDesign),
                Map.entry("feature-rule-count-limit", conventionDesign),
                Map.entry("no-redundant-rule-tags", conventionDesign),
                Map.entry("rule-tag-placement", convention),
                // Advanced Quality (61-63)
                Map.entry("examples-name-when-multiple", convention),
                Map.entry("consistent-scenario-keyword", convention),
                Map.entry("no-duplicate-tags", convention),
                // Ecosystem Parity (64-69)
                Map.entry("required-tags", convention),
                Map.entry("no-restricted-tags", convention),
                Map.entry("name-max-length", convention),
                Map.entry("one-space-between-tags", convention),
                Map.entry("no-restricted-patterns", convention),
                Map.entry("max-tags-per-element", convention),
                // Configurable Thresholds (70-74)
                Map.entry("feature-file-max-lines", conventionPitfall),
                Map.entry("data-table-max-columns", conventionPitfall),
                Map.entry("outline-single-example-row", conventionPitfall),
                // Rules 75-83 (remaining)
                Map.entry("no-duplicate-scenario-bodies", design),
                Map.entry("no-conflicting-tags", convention),
                // Rules 84-86
                Map.entry("max-line-length", convention),
                Map.entry("no-empty-data-table-cells", suspicious),
                Map.entry("examples-constant-column", design)
        );
    }
    ...
    private void configureRule(NewRule rule) {
        ...
        // Set severity from rule metadata (Sonar standard)
        ...
        // Set tags (base + category); default to convention if not in map
        String[] extraTags = RULE_TAGS.getOrDefault(rule.key(), new String[]{"convention"});
        String[] allTags = new String[BASE_TAGS.length + extraTags.length];
        System.arraycopy(BASE_TAGS, 0, allTags, 0, BASE_TAGS.length);
        System.arraycopy(extraTags, 0, allTags, BASE_TAGS.length, extraTags.length);
        rule.setTags(allTags);
