...
A SonarQube plugin that provides comprehensive static analysis of Cucumber Gherkin `.feature` files. It detects structural problems, design issues, style violations, tag misuse, variable errors, step pattern violations, comment markers, and parser errors across **83 analysis rules**. The plugin uses the official Cucumber Gherkin parser and is licensed under the Apache License 2.0.
...
The built-in "Qualimetry Gherkin" quality profile activates **53 rules out of 83** by default. The remaining 30 rules are opt-in and can be activated per project based on team conventions.
